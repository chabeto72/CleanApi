    ...
    public class MapperProfile : Profile
    {
        public MapperProfile()
        {
            ...
            CreateMap<User, GetUserByCodeDocumentNumberModel>()
                .ForMember(target => target.UserId, opt => opt.MapFrom(source => source.UserID))
                .ForMember(target => target.Rol, opt => opt.MapFrom(source => source.RolUserNavigation.Code)).ReverseMap();
